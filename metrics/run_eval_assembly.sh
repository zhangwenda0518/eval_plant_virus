...
for ((i=0; i<TOTAL; i+=BATCH)); do
    ...
    for ((j=i; j<i+BATCH && j<TOTAL; j++)); do
        ...
        (
            t0=$(date +%s)
            # Flat mode: INPUT is a single R1 file, pair with R2
            if $FLAT_MODE; then
                R1="$INPUT"
    ...
    for ((i=0; i<${#MERGE_TASKS[@]}; i+=MERGE_JOBS)); do
        ...
        for ((j=i; j<i+MERGE_JOBS && j<${#MERGE_TASKS[@]}; j++)); do
            ...
            (
                ...
                # ===== MH split+merge =====
                MHSP_DIR="$SDIR/${SNAME}_MH_split_merge"
                [ ! -f "$MHSP_DIR/${SNAME}_MH_split_merge.merged.fasta" ] && {
